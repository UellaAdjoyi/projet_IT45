...
void order_crossover(Individus parent1, Individus parent2, Individus *child) {
    int start = rand() % N;
    int end = rand() % N;
    if (start > end) {
    ...
    int idx = (end + 1) % N;
    for (int i = 0; i < N; i++) {
        int city = parent2.tour[(end + 1 + i) % N];
        if (!included[city]) {
            child->tour[idx] = city;
            idx = (idx + 1) % N;
            included[city] = 1;
...
void genetic_algorithm() {
    ...
    for (int gen = 0; gen < MAX_GEN; gen++) {
        Individus parent1 = selection_tournament();
        Individus parent2 = selection_tournament();
